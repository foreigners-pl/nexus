...
import { getWikiFolderAccessList, removeWikiFolderAccess, shareWikiFolderWithUser } from '@/app/actions/wiki-sharing'
import { getAllUsers } from '@/app/actions/card/assignees'
import type { User } from '@/types/database'

interface ShareWikiModalProps {
  isOpen: boolean
  onClose: () => void
  folderId: string
  folderName: string
...
export function ShareWikiModal({ 
  isOpen, 
  onClose, 
  folderId, 
  folderName, 
  ...
  async function loadAccessList() {
    setLoading(true)
    setError(null)
    const result = await getWikiFolderAccessList(folderId)
  ...
  const handleAddUser = async () => {
    ...
    setSubmitting(true)
    setError(null)
    const result = await shareWikiFolderWithUser(folderId, selectedUser, accessLevel)
  ...
  const handleRemoveAccess = async (userId: string) => {
    setSubmitting(true)
    setError(null)
    const result = await removeWikiFolderAccess(folderId, userId)
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Share "${folderName}"`} maxWidth="lg">
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {loading ? (
            ...
            <p className="text-sm text-neutral-400 text-center py-8">
              Only you have access to this wiki.
            </p>
          ) : (
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {accessList.map((access) => {
                ...
                const canRemove = currentUserId === folderOwnerId && !isOwner
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-neutral-400">
                          {isOwner ? 'Wiki creator' : `Granted ${new Date(access.granted_at).toLocaleDateString()}`}
                        </span>
                      </div>
                    </div>
                    {canRemove && (
